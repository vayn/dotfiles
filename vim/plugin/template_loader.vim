...
"   The default template file name is 'template.extension', for example, if you
"   create a 'foo.c', '~/.vim/templates/template.c' will be loaded. You can set
...
function! LoadTemplate()
  let s:template_file = ""
  let g:template_prefix = exists("g:template_prefix") ? g:template_prefix : 'template'
  let s:ext = expand('%:e')
  if s:ext == ""
    let s:ext = expand('%:t')
  endif
...
function! TemplateReplTags()
  ...
  let s:curpos = searchpos("<T_CURSOR>", '')
  sil! execute '$-1,$s/\n\n//g'
  if !empty(s:curpos)
